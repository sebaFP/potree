...
export class Viewer extends EventDispatcher {
  ...
  fitToScreen(factor = 1, animationDuration = 0) {
    ...
    try {
      let box_scene = new THREE.Box3().setFromObject(this.scene.scene);
      if (box.max.x != Infinity && box_scene.max.x != Infinity) {
        box.max.x = Math.max(box.max.x, box_scene.max.x);
      }
      if (box.max.y != Infinity && box_scene.max.y != Infinity) {
        box.max.y = Math.max(box.max.y, box_scene.max.y);
      }
      if (box.max.z != Infinity && box_scene.max.z != Infinity) {
        box.max.z = Math.max(box.max.z, box_scene.max.z);
      }
      if (box.min.x != -Infinity && box_scene.min.x != -Infinity) {
        box.min.x = Math.min(box.min.x, box_scene.min.x);
      }
      if (box.min.y != -Infinity && box_scene.min.y != -Infinity) {
        box.min.y = Math.min(box.min.y, box_scene.min.y);
      }
      if (box.min.z != -Infinity && box_scene.min.z != -Infinity) {
        box.min.z = Math.min(box.min.z, box_scene.min.z);
      }
    } catch (e) {
      console.log(e);
    }
